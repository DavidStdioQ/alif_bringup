...
def createOemTocPackage(fwsections, outputFile):
    ...
    print("Loading Key1 Cert...")
    key1Cert = getBlob(certPath / 'OEMSBKey1.crt')
    print("Loading Key2 Cert...")
    ...
    print('Unmanaged images certificates')
    ...
    if mPointer != otocStartAddress:
        print("ATOC STARTS IS DIFFERENT THAN THE CALCULATED ONE!!!")
    ...
    print(f"Start of ATOC: {hex(otocStartAddress)}")
    mapf.write('\n  - APP TOC (Table of Content)\n')
    # create the OEM TOC Header
    print("Creating APP TOC...")
    ...
    print('Calculated CRC32: ' + hex(crc32))
    ...
    print(f'mPointer atoc tail - end: {mPointer - oemManagedAreaStartAddress}')

    # check the OEM Package size is the same we calculated before
    if (mPointer - oemManagedAreaStartAddress) != oemManagedAreaSize:
        print('Something is wrong! Final APP Package size is different than the calculated one...')
    ...
    print('Write the APP TOC package to ' + hex(oemManagedAreaStartAddress))
...
def calcOemManagedArea(fwsections):
    ...
    for sec in fwsections:
        ...
        if sec['mramAddress'] == 'none' or sec['mramAddress'] == 0:
            ...
            print(f"Unmanaged: {sec['identifier']}")
            tmp = (sec['identifier'], sec['mramAddress'], fsize + padlen, signature, sec['binary'], compressLabel, encryptionLabel)    # we normaly don't pad unmanaged images, but just in case this change in the future...
            unmanaged.append(tmp)

    print(f"Signed Images: {numSignedImages}")
    print(f"Managed Images: {numManagedImages}")
    print(f"Managed Images total size: {sizeManagedImages:,} bytes")
    ...
    print(f"APP Managed Area size: {oemManagedAreaSize}")

    # calculate OEM Managed Area Starting Addresses
    oemManagedAreaStartAddress = ALIF_BASE_ADDRESS - oemManagedAreaSize
    print(f"APP Package Start Address: {hex(oemManagedAreaStartAddress)}")
    ...
    print("Unmanaged Images - Ordered by MRAM Address")
    for img in unmanaged:
        print(img)
    ...
    if boundary > oemManagedAreaStartAddress:    
        ...
        print(f"Final boundary: {hex(boundary)}")
        print(f"Available MRAM: {oemManagedAreaStartAddress - boundary} bytes")
