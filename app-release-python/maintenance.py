...
def get_power_action(isp):
    ...
    """
    isp_start_no_wait(isp)
    isp_get_power_data_no_wait(isp)
    isp_stop_no_wait(isp)
...
def get_clock_action(isp):
    ...
    """
    isp_start_no_wait(isp)
    isp_get_clock_data_no_wait(isp)
    isp_stop_no_wait(isp)
...
def get_mram_info_action(isp):
    ...
    """
    isp_start_no_wait(isp)
    isp_get_mram_contents_no_wait(isp)
    isp_stop_no_wait(isp)
...
def set_print_toggle_action(isp):
    """
        set print toggle
    """
    isp_start_no_wait(isp)
    isp_set_print_toggle(isp)
    isp_stop_no_wait(isp)

def set_logger_toggle_action(isp):
    """
        set Logger toggle
    """
    isp_start_no_wait(isp)
    isp_set_log_toggle(isp)
    isp_stop_no_wait(isp)
...
def application_mram_erase(isp, address, erase_length, reset):
    ...
    isp_start_no_wait(isp)
    isp_mram_erase_no_wait(isp,address,erase_length,pattern)
    isp_stop_no_wait(isp)
    if reset:
        isp_reset_no_wait(isp)
...
def fast_erase_app_include_start_mram_action(isp):
    """
        fast_erase_app_include_start_mram_action
            ...
            Plus the beggining of MRAM (0x8000-0000)
    """
    ...
    application_mram_erase(isp, address, erase_length, False)  # this function can't be called twice as target stops to respond

    # erase start of mram (remove residuals from applications that start at 0x8000-0000)
    address = OEM_BASE_ADDRESS
    erase_length = FAST_ERASE_SIZE
    application_mram_erase(isp, address, erase_length, True)  # this function can't be called twice as target stops to respond
...
def get_log_data_action(isp):
    ...
    """
    isp_start_no_wait(isp)
    log_data = isp_get_log_data_no_wait(isp)
    if log_data != None:
