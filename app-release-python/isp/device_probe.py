...
"""
import struct
from isp_core import isp_start_no_wait
from isp_core import isp_stop_no_wait
...
class device_get_attributes:
    ...
    def __init__(self, isp):
        ...
        self.revision = self.__get_device_revision(isp)
        # probe and set device bootloader stage
        self.stage = self.__get_device_stage(isp)

    def __get_device_revision(self, isp):
        """
        ...
        if cmd == ISP_COMMAND_DATA_RESPONSE:
            lst = message[ISP_PACKET_DATA_FIELD: ISP_PACKET_DATA_FIELD+4]
