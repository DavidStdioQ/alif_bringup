...
isp_command_lookup = {
            ...
            ISP_COMMAND_DEBUG_ENABLE        : "COMMAND_DEBUG_ENABLE  ",
...
isp_subcommand_lookup = {
            ...
            ISP_SET_TOGGLE_PRINTING         : "SET_TOGGLE_PRINTING   ",
            ISP_SET_TOGGLE_LOGGING          : "SET TOGGLE LOGGING    "
}
...
def isp_reset_no_wait(isp):
    """
        isp_reset_no_wait
    """
    isp_build_packet(isp, ISP_COMMAND_RESET_DEVICE)

def isp_secure_debug_no_wait(isp, auth_token, data):
    """
...
def isp_download_start_no_wait(isp, address, size, auth_token):
    """
...
def isp_download_data_no_wait(isp, data):
    """
    ...
    if command == ISP_COMMAND_NAK and isp.getExit():
...
def isp_download_done_no_wait(isp):
    """
        isp_download-done_no_wait
        Does not wait for reader task
    """
    isp_build_packet(isp, ISP_COMMAND_DOWNLOAD_DONE)

def isp_enquiry_no_wait(isp):
    """
        isp_enquiry
    """
...
def isp_get_revision_no_wait(isp):
    """
...
def isp_get_baud_rate_no_wait(isp):
    """
        isp_get_baud_rate
    """
    baud_rate = 0
...
def isp_get_toc_data_no_wait(isp):
    """
        isp_get_toc_data
    """
...
def isp_get_power_data_no_wait(isp):
    """
        isp_get_power_data
    """
...
def isp_get_clock_data_no_wait(isp):
    """
        isp_get_power_data
    """
...
def isp_get_mram_contents_no_wait(isp):
    """
...
# DEBUG lives here
#        print("{:4d} 0x{:08x} ".format(each_word,value))
    trace_buffer_decode(trace_buffer)
...
def isp_set_log_toggle(isp):
    """
        SET capability - LOGGER toggle
    """
#    delay = 0
    response = isp_build_packet(isp, ISP_COMMAND_SET, [ISP_SET_TOGGLE_LOGGING])
    ...
    return
def isp_set_print_toggle(isp):
    """
        SET capability - PRINT toggle
    """
    delay = 0
    response = isp_build_packet(isp, ISP_COMMAND_SET, [ISP_SET_TOGGLE_PRINTING], delay)
...
def isp_read_mram(isp,word_offset):
    """
        isp_read_mram
            retrieve mram @ word_offset
    """
    cmd_packet = [0x03, ISP_COMMAND_MRAM_READ]
    cmd_packet = cmd_packet + list(int_to_bytes(word_offset))
    cmd_packet = cmd_packet + list(b'\x10')
    cmd_packet[0] = len(cmd_packet) + 1
    cmd_packet = isp.checkSum(cmd_packet)  # add the checksum packet
    ...
    for each_word in range(2,18,4):
        mram_value = message[each_word:each_word+4]
        value = int.from_bytes(mram_value,byteorder='little')
        isp_print_color("blue","0x{:x} ".format(value))
    print(" ")

def isp_set_baud_rate_no_wait(isp, baud_rate):
    """
        isp_get_baud_rate
    """
...
def isp_mram_write_no_wait(isp, offset, data):
    """
        isp_Mram_Write
        - send data to write to the MRAM - SEROM style
        (used by icv-recovery.py and recovery.py)
    """
    isp_build_packet(isp, ISP_COMMAND_MRAM_WRITE,
                     [offset, bytearray(data)])

def isp_burn_mram_no_wait(isp, address, file_size, auth_token):
    """
        isp_burn_mram
    """
    ...
    if auth_token != 0x0:
        subCmd.append(auth_token)
    isp_build_packet(isp, ISP_COMMAND_BURN_MRAM, subCmd)


def isp_mram_erase_no_wait(isp, address, size=0,pattern=0x00000000):
    """
        isp_erase_mram
...
def isp_set_maintenance_flag_no_wait(isp):
    """
...
def isp_get_log_data_no_wait(isp):
    """
        isp_get_log_data
    """
...
def isp_get_seram_update_address_no_wait(isp):
    """
