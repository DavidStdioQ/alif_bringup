...
def globalToLocalAddress(globalAddress):
    ...
    if utils.config.DEVICE_FEATURE_SET == 'Fusion':
        if utils.config.DEVICE_REVISION == 'B0':
            regions = regions_B0
            deltas = deltas_B0
        else:
            regions = regions_Ax
...
def createContentCerts(fwsections, prefix):
    ...
    for sec in fwsections:
        ...
        cert_sb_content_util.main(["-c", 'cfg/'+configFile, "-l", '../build/logs/ICVSBContent.log'])
        os.chdir(os.getcwd() + '/../')
    ...
    for sec in fwsections:
        ...
        if 'LOAD' in sec['flags']:
            if sec['loadAddress'] == 'none':
                ...
                print('Please correct the configuration in image '
                      + sec['identifier'])
                sys.exit(EXIT_WITH_ERROR)
