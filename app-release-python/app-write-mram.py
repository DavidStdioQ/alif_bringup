...
# 0.20.003     remove maintenace mode output unless -v is used)
...
TOOL_VERSION ="0.21.000"        # Define Version constant for each separate tool
...
def app_mram_erase(isp,args, ALIF_BASE_ADDRESS, ALIF_MRAM_SIZE):
    ...
    if argc >= 3:
        ...
        isp_mram_erase_no_wait(isp,address,erase_len,pattern)
...
def main():
    ...
    parser.add_argument("-x", "--exit", default=True, 
                        help="exit on NAK", action="store_true")
...
#        dynamic_baud_rate_switch = args.switch
    ...
    if method == 'jtag' and not args.erase:     # erase via jtag not yet supported!
        ...
        #jlink.reset()  # the reset causes an issue and the board will beed to power cycle...
    ...
    isp_stop_no_wait(isp)              # Stop ISP Sequence
